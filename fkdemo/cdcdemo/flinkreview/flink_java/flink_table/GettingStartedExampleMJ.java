...
public class GettingStartedExampleMJ {
    public static void main(String[] args) throws Exception {
        ...
        final Table rawCustomers = env.fromValues(Row.of(
                "Guillermo Smith",
        ...
        final Table namedCustomers = truncatedCustomers.as("name", "date_of_birth", "street", "zip_code", "city",
                "gender", "has_newsletter");
        // regesiter a view temporarily
        env.createTemporaryView("customers", namedCustomers);

        // use SQL whenever you like, call execute() and print() to get insights
        env.sqlQuery(
                "SELECT COUNT(*) AS `number of customers`, AVG(YEAR(date_of_birth) as `average birth year` FROM `customers`))")
                .execute().print();
        ;

        // or further transform the data using the fluent Table API
        final Table youngCustomers = env.from("customers").filter($("gender").isNotNull())
        ...
        try (CloseableIterator<Row> iterator = youngCustomers.execute().collect()) {
            ...
            expectedOutput.add(
                    Row.of("GUILLERMO SMITH",
                            LocalDate.parse("1992-12-12"),
                            "4081 VALLEY ROAD, 08540, NEW JERSEY"));

            expectedOutput.add(Row.of("JOHN TURNER",
                    LocalDate.parse("1982-10-02"),
    ...
    public static class AddressNormalizer extends ScalarFunction {
        ...
        private String normalize(String s) {
            // 任何非单词字符和空白字符都变成" "
            return s.toUpperCase().replaceAll("\\W", " ").replaceAll("\\s+", " ");
        }
    }
